...
void Preprocess::load_projected_data(const std::string& path)
{
	std::string file = path + "_new_pca";
	std::ifstream in(file.c_str(), std::ios::binary);
	while (!in) {
		printf("Fail to find data file!\n");
		exit(0);
	...
	in.read((char*)header, sizeof(header));
	assert(header[0] == sizeof(float));
	projected_data.N = header[1];
	projected_data.dim = header[2];

	projected_data.val = new float* [projected_data.N];
	for (int i = 0; i < projected_data.N; ++i) {
		projected_data.val[i] = new float[projected_data.dim];
		//in.seekg(sizeof(float), std::ios::cur);
		in.read((char*)projected_data.val[i], sizeof(float) * header[2]);
	}
	...
	//}
	int MaxQueryNum = 200;
	projected_data.query = projected_data.val;
	projected_data.val = &(projected_data.query[MaxQueryNum]);
	projected_data.N -= MaxQueryNum;

	std::cout << "Load from projected new file: " << file << "\n";
	std::cout << "N=    " << projected_data.N << "\n";
	std::cout << "dim=  " << projected_data.dim << "\n\n";
