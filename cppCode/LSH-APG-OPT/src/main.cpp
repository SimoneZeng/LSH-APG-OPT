...
int main(int argc, char const* argv[])
{
	...
	if (argc > 5) L = std::atoi(argv[4]);
	if (argc > 6) K = std::atoi(argv[5]);
	if (argc > 7) T = std::atoi(argv[6]);
	if (argc > 8) efC = std::atoi(argv[7]);
	if (argc > 9) pC = std::atof(argv[8]);
	if (argc > 10) pQ = std::atof(argv[9]);
	if (argc > 11) _lsh_UB = std::atoi(argv[10]);
	if (argc == 1) {
		...
		std::string data_fold = "/data1/dlw/ANN/LSH-APG-OPT/dataset/", index_fold = "./indexes/";
	#else
	...
	// set threshold
	divG->coeff_threshold = 1 / threshold;
	...
	fsG->coeff_threshold = 1 / threshold;
