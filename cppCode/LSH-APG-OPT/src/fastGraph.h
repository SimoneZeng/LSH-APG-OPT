 ...
 public:
    ...
    int u = 0;
 	...
 	fastGraph(divGraph* divG)
 	{
 		...
 		dataset = divG->myData;
        projected_dataset = divG->myProjectedData;
 		dim = divG->dim;
...
#include "divGraph.h"
    ...
    void searchLSHQuery(queryN * q, std::priority_queue<Res>& candTable, std::vector<bool>& flag_)
    {
 	...
 	void knn(queryN* q) {
        ...
        _mm_prefetch((char*)(q->queryPoint), _MM_HINT_T0);
#endif
        // cout<< "candTable.size() = " << candTable.size() << endl;
        ...
        while (!candidate_set.empty()) {
           ...
           //_mm_prefetch(links + (*(data + 1)) * size_data_per_element_ + offsetData_, _MM_HINT_T0);
            _mm_prefetch((char*)(dataset[data[1]]), _MM_HINT_T0);
            _mm_prefetch((char*)(projected_dataset[data[1]]), _MM_HINT_T0);
            _mm_prefetch((char*)(data + 1), _MM_HINT_T0);
#endif
            ...
            for (size_t j = 1; j <= size; j++) {
               ...
               //_mm_prefetch((char*)(dataset[*(data + j + 1)]), _MM_HINT_T0);
                ...
                if (!flag_[candidate_id]) {
                    ...
                    // cout << "projected_dataset" << endl;
                    // cout << projected_dataset[candidate_id][0] << " " << projected_dataset[candidate_id][1] << endl;
                    // cout << "q" << endl;
                    // cout << q->projected_queryPoint[0] << " " << q->projected_queryPoint[1] << endl;

                    // if (0 || cal_dist(q->hashval, hashval[candidate_id], lowDim) > lowerBound) {
//                     if (0 || cal_dist(q->projected_queryPoint, projected_dataset[candidate_id], lowDim) > lowerBound){
//                         q->prunings++;
//                     }
//                     else{
//                         float* currObj1 = dataset[*(data + j)];
...
// #ifdef USE_SSE
//                             _mm_prefetch((char*)(dataset[candidate_set.top().second]), _MM_HINT_T0);
//                             _mm_prefetch((char*)(projected_dataset[candidate_set.top().second]), _MM_HINT_T0);
// #endif
...
//                     }
                    ...
                    if (0 || cal_dist(q->projected_queryPoint, projected_dataset[candidate_id], lowDim) * coeff_threshold < lowerBound) {
                        float* currObj1 = dataset[*(data + j)];
                        dist_t dist = cal_dist(q->queryPoint, currObj1, dim);
                        q->cost++;
                        if (top_candidates.size() < ef || lowerBound > dist) {
